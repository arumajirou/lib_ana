...
def _init_state() -> None:
    ...
    if "initialized" not in st.session_state:
        ...
        st.session_state.analysis = (
            None  # dict(summary, nodes, edges, tables, indexes, classified)
        )
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        lib_name = st.selectbox(
            ...
            options=libs,
            index=(
                libs.index(st.session_state.last_selected.get("library", libs[0]))
                if libs
                else 0
            ),
        )

        st.session_state.open_new_tab = st.checkbox(
            "外部URLを新規タブで開く", value=st.session_state.open_new_tab
        )
        st.session_state.color_tables = st.checkbox(
            "表を色分けする", value=st.session_state.color_tables
        )
        st.session_state.enable_online_lookup = st.checkbox(
            "PyPI/GitHub/HF をオンライン探索",
            value=st.session_state.enable_online_lookup,
        )
