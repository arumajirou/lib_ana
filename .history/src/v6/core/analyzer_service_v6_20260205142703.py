...
def _import_analyzer() -> Any:
    """
    解析器の実体は環境により置き場が揺れるので、
    v5 / v4 / library_explorer_v4 を順に探索する。
    """
    candidates = [
        ("v5.analyzer_v5", "LibraryAnalyzerV5"),
        ("v4.analyzer_v4", "LibraryAnalyzerV4"),
        # もし v4 が package でなくても、namespace package として import できる場合がある
...
def _cached_analyze(lib_name: str) -> Tuple[Dict[str, Any], pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if isinstance(out, tuple) and len(out) >= 3:
        summary = out[0] or {}
        df_nodes = out[1] if isinstance(out[1], pd.DataFrame) else pd.DataFrame(out[1])
        df_edges = out[2] if isinstance(out[2], pd.DataFrame) else pd.DataFrame(out[2])
        if len(out) >= 4:
            df_errors = out[3] if isinstance(out[3], pd.DataFrame) else pd.DataFrame(out[3])
    elif isinstance(out, dict):
        summary = out.get("summary", {}) or {}
        df_nodes = out.get("nodes", pd.DataFrame())
        df_edges = out.get("edges", pd.DataFrame())
        df_errors = out.get("errors", pd.DataFrame())
    else:
...
def analyze_library_with_progress(lib_name: str, deep_param_inspect: bool = False) -> Dict[str, Any]:
