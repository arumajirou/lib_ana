...
# --- ここが追加：レガシー(v4/v5)のトップレベルimportを成立させる ---
def _bootstrap_legacy_sys_path() -> None:
    ...
    legacy_dirs = [
        ...
        ),  # models_v4 / taxonomy_v4 / package_catalog_v4 等がいる
        os.path.join(
            src_root, "v5"
        ),  # models_v5 等がトップレベルimportされる場合に備える
        src_root,  # 念のため
    ]

    # 先頭優先で入れる（同名モジュールがあるときにレガシー側を先に拾う）
...
def _cached_analyze(
    ...
    if isinstance(out, tuple) and len(out) >= 3:
        ...
        if len(out) >= 4:
            df_errors = (
                out[3] if isinstance(out[3], pd.DataFrame) else pd.DataFrame(out[3])
            )
