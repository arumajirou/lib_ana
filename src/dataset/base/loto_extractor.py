...
class LotoExtractor:
    ...
    def _normalize_loto_df(self, df: pd.DataFrame, loto: str) -> pd.DataFrame:
        ...
        # ------------------------------
        # ボーナス数字の「桁」を特徴量として追加
        # 例: B1=37 → B1_TENS=3, B1_ONES=7
        # ※後段で hist_ プレフィックスが付与され、hist_b1_tens / hist_b1_ones になる
        # ------------------------------
        bonus_cols = sorted([c for c in df.columns if re.fullmatch(r"B\d+", str(c))])
        for bcol in bonus_cols:
            try:
                # pandasのnullable Int64 を想定
                s = df[bcol].astype("Int64")
                df[f"{bcol}_TENS"] = (s // 10).astype("Int64")
                df[f"{bcol}_ONES"] = (s % 10).astype("Int64")
            except Exception:
                # 失敗してもETL全体を止めない（欠損扱い）
                df[f"{bcol}_TENS"] = pd.Series([pd.NA] * len(df), dtype="Int64")
                df[f"{bcol}_ONES"] = pd.Series([pd.NA] * len(df), dtype="Int64")
